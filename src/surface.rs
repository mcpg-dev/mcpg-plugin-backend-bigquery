...
mod tests {
    ...
    fn static_uri_wins_over_argument() {
        let args = json!({ "uri": "bigquery://from-arg" });
        assert_eq!(
            resolve_resource_uri(Some("bigquery://static"), &args),
            Some("bigquery://static")
        );
    }

    #[test]
    fn falls_back_to_argument_uri() {
        let args = json!({ "uri": "bigquery://docs/readme" });
        assert_eq!(
            resolve_resource_uri(None, &args),
            Some("bigquery://docs/readme")
        );
    }
    ...
    fn resource_body_satisfies_decoder_shape() {
        ...
        let body = resource_contents_body("bigquery://docs", &rows);
        let contents = body["contents"].as_array().expect("contents array");
        assert_eq!(contents.len(), 1);
        assert_eq!(contents[0]["uri"], json!("bigquery://docs"));
    ...
    fn page_slices_full_result_and_advances_cursor() {
        let all: Vec<Value> = (0..5)
            .map(|i| json!({ "uri": format!("bigquery://item/{i}"), "name": format!("Item {i}") }))
            .collect();
        let p0 = page_from_full_result(&all, 0, 2);
        assert_eq!(p0.resources.len(), 2);
        assert_eq!(p0.resources[0].uri, "bigquery://item/0");
        ...
        let past = page_from_full_result(&all, 99, 2);
    ...
    fn page_skips_rows_without_uri() {
        let all = vec![
            json!({ "name": "no uri" }),
            json!({ "uri": "bigquery://ok" }),
        ];
        let page = page_from_full_result(&all, 0, 10);
        assert_eq!(page.resources.len(), 1);
        assert_eq!(page.resources[0].uri, "bigquery://ok");
    }
}
